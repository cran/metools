...
#'
#' @description
#' mp.ts make plot in time serie format with one or more series.
...
#'
mp.ts=function(object,xaxis,yaxis,dateformat='%Y-%m',datebreaks="1 month",ybreaks= 10,percent=FALSE,
    ...
    g=object
    if(percent==TRUE){
        g= g + ggplot2::scale_x_date(labels = scales::date_format(dateformat), breaks = metools::p.seqdatebreaks(xaxis,datebreaks),limits=xlim)+
