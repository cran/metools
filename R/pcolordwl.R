...
#'
#' @return Return ordered bar plot with legend.
#' @export
#'
#' @examples
...
#'
p.col_ord_wl= function(data,xaxis,yaxis,ybreaks= 10,percent=FALSE,dec=FALSE,
                    yaccuracy=0.01,ydecimalmark=".",title='Title',xlab='X axis',
                   ...
                   legdir='horizontal',legcol='white',legspa=1,legvjust=0.5,
                   colors=grDevices::rainbow(length(xaxis),v = 0.7)
){
    if(dec==FALSE & percent==FALSE){
        g=(ggplot2::ggplot(stats::na.exclude(data),ggplot2::aes(x=stats::reorder(xaxis,yaxis),y=yaxis),colour= base::factor(xaxis))+
               ggplot2::geom_col(ggplot2::aes(fill = base::factor(xaxis,levels=xaxis)),color=cbserie,lwd=lwdserie)
    ...
    if(dec==FALSE & percent==TRUE){
        g=(ggplot2::ggplot(stats::na.exclude(data),ggplot2::aes(x=stats::reorder(xaxis,yaxis),y=yaxis),colour= base::factor(xaxis))+
               ggplot2::geom_col(ggplot2::aes(fill = base::factor(xaxis,levels=xaxis)),color=cbserie,lwd=lwdserie)
           +ggplot2::labs(title = title, y=ylab, x=xlab,subtitle = stitle,caption=note,fill=legtitle)
           +ggplot2::scale_x_discrete(name=NULL,labels=NULL)+ggplot2::scale_y_continuous(labels = scales::label_percent(),breaks = scales::breaks_extended(ybreaks))
           + ggplot2::theme(legend.title.align = 0.5, legend.text=ggplot2::element_text(face = 'bold',colour = cleg,size = legsize,margin=ggplot2::margin(b=legspa,unit='cm'),vjust = legvjust),
                            legend.key.height = ggplot2::unit(legheight,'cm'),legend.position = legpos,legend.background = ggplot2::element_rect(fill = legcol),
                            legend.key=ggplot2::element_rect(fill= legcol,colour=legcol,color=legcol),
                            title= ggplot2::element_text(angle = 0, hjust = 0.5,size = wordssize,face = 'bold',color=ctitles),
    ...
    if(dec==TRUE & percent==TRUE){
        g=(ggplot2::ggplot(stats::na.exclude(data),ggplot2::aes(x=stats::reorder(xaxis,-yaxis),y=yaxis),colour= base::factor(xaxis))+
               ggplot2::geom_col(ggplot2::aes(fill = base::factor(xaxis,levels=xaxis)),color=cbserie,lwd=lwdserie)
           +ggplot2::labs(title = title, y=ylab, x=xlab,subtitle = stitle,caption=note,fill=legtitle)
           +ggplot2::scale_x_discrete(name=NULL,labels=NULL)+ggplot2::scale_y_continuous(labels = scales::label_percent(),breaks = scales::breaks_extended(ybreaks))
           + ggplot2::theme(legend.title.align = 0.5, legend.text=ggplot2::element_text(face = 'bold',colour = cleg,
                                                                      size = legsize,margin=ggplot2::margin(b=legspa,unit='cm'),vjust = legvjust),
                   legend.key.height = ggplot2::unit(legheight,'cm'),legend.position = legpos,legend.background = ggplot2::element_rect(fill = legcol),
                   legend.key=ggplot2::element_rect(fill= legcol,colour=legcol,color=legcol),
                   title= ggplot2::element_text(angle = 0, hjust = 0.5,size = wordssize,face = 'bold',color=ctitles),
                   plot.title= ggplot2::element_text(angle = 0, vjust = 0.5, hjust=0.5,size = titlesize,face = 'bold',color = ctitles),
